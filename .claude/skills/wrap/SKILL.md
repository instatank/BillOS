---
name: wrap
description: Session-wrap ritual - run before ending any session that shipped commits (the Stop hook will nudge once if forgotten). Reconciles SESSION_HANDOFF.md against reality, appends friction cards to LEARNINGS.md, asks the founder the teach-back/transfer questions, quizzes one old card. Also triggered by "wrap and teach" or "wrap up".
---
...
1. **Reconcile the handoff doc** (`SESSION_HANDOFF.md`): verify every stated fact against reality — current `sw.js` `VERSION`, last commit, branch, what shipped vs pending. Fix drift in place (PLAYBOOK Rule 6).
...
5. **Recap:** produce the `wrap and teach` session recap in the format defined in CLAUDE.md (this session only, no padding).
